			...
			<table class="table dataTable no-footer" style="width:100%">
				<thead>
				   <tr>
					  ...
					  <th class="sorting {{ $sortBy == 'new_arrivals' ? ( $sortOrder == 'asc' ? 'sorting_asc' : 'sorting_desc' ) : '' }}" wire:click="fetchOrder('new_arrivals', '{{ $sortBy == 'new_arrivals' ? ( $sortOrder == 'asc' ? 'desc' : 'asc' ) : 'asc' }}')">New Arrivals</th> 
				   ...
				   <tr>
					  ...
					  <td>
						 <div class="btn-group mb-1">
							<a href="javascript:void(0)" data-modal-title="{{ $product->name }}" onclick="loadDetailModal(this,'{{ route('admin.products.show', $product->id) }}')" class="btn btn-outline-success">Info</a>
							
							<button type="button" class="btn btn-outline-success dropdown-toggle dropdown-toggle-split" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false" data-display="static"><span class="sr-only">Info</span></button>
