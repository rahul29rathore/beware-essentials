<div class="content">
  <div class="breadcrumb-wrapper d-flex align-items-center justify-content-between">
	 <div>
		<h1>Contact Leads</h1>
		<p class="breadcrumbs"><span><a href="{{ route('admin.dashboard') }}">Dashboard</a></span> <span><i class="mdi mdi-chevron-right"></i></span>Contact Leads</p>
	 </div>
  </div>
  <div class="row">
	 <div class="col-12">
						...
						<table class="table dataTable no-footer" style="width:100%">
							<thead>
							   <tr>
								  ...
								  <th class="sorting {{ $sortBy == 'subject' ? ( $sortOrder == 'asc' ? 'sorting_asc' : 'sorting_desc' ) : '' }}" wire:click="fetchOrder('subject', '{{ $sortBy == 'subject' ? ( $sortOrder == 'asc' ? 'desc' : 'asc' ) : 'asc' }}')">Subject</th>
								  ...
								  <th class="sorting {{ $sortBy == 'contact_no' ? ( $sortOrder == 'asc' ? 'sorting_asc' : 'sorting_desc' ) : '' }}" wire:click="fetchOrder('contact_no', '{{ $sortBy == 'contact_no' ? ( $sortOrder == 'asc' ? 'desc' : 'asc' ) : 'asc' }}')">Mobile No.</th>
								  <th class="sorting {{ $sortBy == 'customer_type' ? ( $sortOrder == 'asc' ? 'sorting_asc' : 'sorting_desc' ) : '' }}" wire:click="fetchOrder('customer_type', '{{ $sortBy == 'customer_type' ? ( $sortOrder == 'asc' ? 'desc' : 'asc' ) : 'asc' }}')">Customer type</th>
								  <th>Message</th>
